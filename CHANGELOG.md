...
- examples/operator-intra.yaml file to bundle all the rbac, crds, etc to run the
  operator
- EtcdURL field to controllersetcontroller spec. default: etcd-knopic:2379
...
### Removed
- `ControllerEndpoint` and `DefaultController` from the KnopicNodeSet spec
...
- Opt-in node label for nodes is now `linstor.linbit.com/knopic-node=true`
